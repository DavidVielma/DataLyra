...
import { Navbar } from "@/components/layout/Navbar"; // Ensure navbar is present if used in route wrapper, but here it's standalone page component
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 relative z-10">
            {steps.map((step, idx) => (
              ...
              >
                <div className={`w-48 h-48 rounded-full border-2 ${step.color} ${step.bg} flex flex-col items-center justify-center p-6 mb-8 relative transition-transform duration-500 group-hover:scale-105 backdrop-blur-sm shadow-xl dark:shadow-[0_0_30px_rgba(0,0,0,0.3)] bg-background/50`}>
                ...
                {idx < steps.length - 1 && (
                  <div className="md:hidden mb-8 text-muted-foreground">
                    <ArrowRight className="h-8 w-8 rotate-90" />
                  </div>
                )}
                 {idx < steps.length - 1 && (
                  <div className="hidden md:block absolute top-1/2 left-[30%] translate-x-12 -translate-y-12 text-muted-foreground">
